...
func isIdentifierStart(char rune) bool {
	return unicode.IsLower(char) || unicode.IsUpper(char)
}
...
func (l *Lexer) readString() (string, error) {
	var sb strings.Builder
	l.readChar() // reading the first character
	for l.char != '"' {
		if l.char == 0 {
		...
		if l.char == '\x00' {
			return "", fmt.Errorf("Null character (\\0) in string constant")
		}
...
func (l *Lexer) NextToken() Token {
	...
	case l.char == '(':
		if l.peekChar() == '*' {
			...
			l.readChar()
			l.readChar()
			openCount := 1 // count of openings (to handle nested comments)
			// (*(*(*)
			for openCount != 0 {
				if l.char == 0 {
					tok.Type = ERROR
					tok.Literal = "Unterminated comment"
					return tok
	...
	case isIdentifierStart(l.char):
		...
		case "not":
			tok.Type = NOT
		case "true", "false":
			if unicode.IsLower(rune(identifier[0])) {
				tok.Type = BOOL_CONST
			} else {
				tok.Type = TYPEID
			}
